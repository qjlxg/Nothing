...
FAKE_IPS = (
    "8.8.8.8; 8.8.4.4; 4.2.2.2; 4.2.2.1; 114.114.114.114; 127.0.0.1; 0.0.0.0".split(
        "; "
    )
)
...
BANNED_WORDS = b64decodes(
    "5rOV6L2uIOi9ruWtkCDova4g57uDIOawlCDlip8gb25ndGFpd2Fu"
).split()
...
class Node:
    ...
    def url(self) -> str:
        ...
        if self.type == "ss":
            ...
            return (
                f"ss://{passwd}@{data['server']}:{data['port']}#{quote(data['name'])}"
            )
        if self.type == "ssr":
            ret = (
                ":".join(
                    [
                        str(self.data[_])
                        for _ in ("server", "port", "protocol", "cipher", "obfs")
                    ]
                )
        ...
        try:
            if self.type == "ssr":
                if "obfs" in self.data and self.data["obfs"] not in CLASH_SSR_OBFS:
                    return False
                if (
                    "protocol" in self.data
                    and self.data["protocol"] not in CLASH_SSR_PROTOCOL
                ):
                    return False
            if (
                "plugin-opts" in self.data
                and "mode" in self.data["plugin-opts"]
            ...
            if self.url.startswith("dynamic:"):
                ...
                if "#" in self.url:
                    ...
                    if "ignore" in self.cfg:
                        self.cfg["ignore"] = [
                            _ for _ in self.cfg["ignore"].split(",") if _.strip()
                        ]
    ...
    def parse(self) -> None:
        ...
        try:
            ...
            if "max" in self.cfg and len(sub) > self.cfg["max"]:
                exc_queue.append(
                    f"此订阅有 {len(sub)} 个节点，最大限制为 {self.cfg['max']} 个，忽略此订阅。"
                )
                self.sub = []
            elif sub and "ignore" in self.cfg:
                if isinstance(sub[0], str):
                    self.sub = [
                        _ for _ in sub if _.split("://", 1)[0] not in self.cfg["ignore"]
                    ]
                elif isinstance(sub[0], dict):
                    self.sub = [
                        _ for _ in sub if _.get("type", "") not in self.cfg["ignore"]
                    ]  # type:ignore
                else:
                    self.sub = sub
            else:
...
def main():
    ...
    for hashp, p in merged.items():
        try:
            if hashp in used:
                ...
                p.data["name"] = (
                    ",".join([str(_) for _ in sorted(list(used[hash(p)]))])
                    + "|"
                    + p.data["name"]
                )
            if p.supports_ray():
    ...
    for p in unknown:
        txt += p + "\n"
    print(
        f"共有 {len(merged)-unsupports} 个正常节点，{len(unknown)} 个无法解析的节点，共",
        len(merged) + len(unknown),
    ...
    try:
        ...
        for ctg, proxies in ctg_nodes_meta.items():
            with open(
                "snippets/nodes_" + ctg + ".meta.yml", "w", encoding="utf-8"
            ) as f:
                yaml.dump({"proxies": proxies}, f, allow_unicode=True)
    ...
    for p in merged.values():
        if p.supports_meta():
            if (
                "client-fingerprint" in p.data
                and p.data["client-fingerprint"] == global_fp
            ):
    ...
    with open("snippets/nodes.yml", "w", encoding="utf-8") as f:
        f.write(
            yaml.dump({"proxies": proxies}, allow_unicode=True).replace("!!str ", "")
        )
